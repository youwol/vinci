...
export class Seidel implements Solver {
    ...
    set messageCB(m: MessageCallback) {
        this.msgCB = m
    }
    set warningCB(m: MessageCallback) {
        this.warCB = m
    }
    set errorCB(m: MessageCallback) {
        this.errCB = m
    }
    set iterationCB(m: IterationCallback) {
        this.iterCB = m
    }
    ...
    run() {
        if (!this.valid_) {
            this.errCB(
                'Cannot run the solver, initiallization was stopped by user',
            )
            return
        }
