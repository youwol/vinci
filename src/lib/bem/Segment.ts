...
export class Segment {
    ...
    bcType(i: number) {
        return this.bcType_[i]
    }
    setBcType(i: number, bc: BC) {
        this.bcType_[i] = bc
    }

    get bcValues() {
        return this.bcValue_
    }
    bcValue(i: number) {
        return this.bcValue_[i]
    }
    setBcValue(i: number, v: number) {
        this.bcValue_[i] = v
    }
    setBcValues(v: Vector) {
        this.bcValue_[0] = v[0]
        this.bcValue_[1] = v[1]
    }
    addBcValues(v: Vector) {
        this.bcValue_[0] += v[0]
        this.bcValue_[1] += v[1]
    }
    ...
    get dof() {
        return (
            (this.bcType_[0] === BC.Traction ? 1 : 0) +
            (this.bcType_[1] === BC.Traction ? 1 : 0)
        )
    }
    ...
    tooClose(p: Point, delta = 1): boolean {
        return (
            distToSegment2(p, this.begin, this.end) < delta ** 2 * this.l2 ** 2
        )
    }
    ...
    tractionIcAt(p: Point): TractionCoeff {
        ...
        return [
            [
                (stress[1][0] - stress[0][0]) * sincos +
                    stress[2][0] * (cos2 - sin2),
                (stress[1][1] - stress[0][1]) * sincos +
                    stress[2][1] * (cos2 - sin2),
            ],
            [
                stress[0][0] * sin2 -
                    2 * stress[2][0] * sincos +
    ...
    private displAndStressCoeff(p: Point): {
        displ: DisplCoeff
        stress: StressCoeff
    } {
        if (this.mat === undefined) {
