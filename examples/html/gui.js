...
function connectGui() {
    ...
    upload.onchange = () => {
        upload.files[0].arrayBuffer().then((arrayBuffer) => {
            ...
            const dfs = filter.decode(new TextDecoder().decode(arrayBuffer), {
                shared: false,
                merge: true,
            })
            // console.log(dfs)
    ...
    mod.add(model, 'gridSampling', 10, 500, 1)
        .name('Grid sampling')
        .onChange((value) => {
            gridSampling = value
    ...
    kepler.getColorMapNames().forEach((name) => {
        kepler.ColorMap.addColorMap(
            name,
            kepler.getColorMap(name, 40, false).colors,
        )
    })
    const colorTables = kepler.colorMapNames()
    display
        .add(isoParams, 'lut', colorTables)
        .name('Color table')
        .onChange((value) => {
            isoParams.lut = value
            repaint()
    ...
    display
        .add(surfParams, 'pointSize', 1, 10, 1)
        .name('Points size')
        .onChange((value) => {
            // surfaceInfo.points.show = value
            repaint()
    ...
    */
    ...
    stream
        .add(streamLines, 'show')
        .name('Show')
        .onChange((value) => repaint())
        .disable()
    stream
        .add(streamLines, 'width')
        .name('Width', 0.0001, 0.01, 0.001)
        .onChange((value) => repaint())
    stream
        .addColor(colorStream, 'string')
        .name('Color')
        .onChange((value) => {
    ...
    stream
        .add(streamLines, 'maximumPointsPerLine', 5, 200, 5)
        .onChange((value) => repaint())
    stream.add(streamLines, 'dSep', 0.01, 1, 0.1).onChange((value) => repaint())
    stream
        .add(streamLines, 'timeStep', 0.01, 0.5, 0.01)
        .onChange((value) => repaint())
    stream
        .add(streamLines, 'dTest', 0.01, 0.5, 0.01)
        .onChange((value) => repaint())
    stream
    ...
    vectors
        ...
        .listen()
    vectors
        .add(surfaceInfo.vectors, 'scale', 0.01, 1, 0.01)
        .name('Scale')
        .onChange((value) => repaint())
    vectors
        .add(surfaceInfo.vectors, 'normalize')
        .name('Normalize')
        .onChange((value) => repaint())
    ...
    displayL
        .add(faultParams, 'show', 1, 10, 1)
        .name('Show')
        .onChange((value) => {
            repaint()
        })
    displayL
        .add(faultParams, 'width', 0.0001, 0.05, 0.001)
        .name('Line width')
        .onChange((value) => {
            repaint()
        })
    displayL
        .addColor(colorLine, 'string')
        .name('Color')
        .onChange((value) => {
            repaint()
        })
    ...
    displayL
        .add(faultParams, 'showPoints')
        .name('Show points')
        .onChange((value) => {
            repaint()
        })
    displayL
        .add(faultParams, 'pointSize', 1, 10, 1)
        .name('Points size')
        .onChange((value) => {
            repaint()
        })
